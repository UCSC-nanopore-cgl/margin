...
### Docker ###
There is a margin Docker container provided at `docker.io/kishwars/margin_polish`.  To run, the directory with your input files must be mounted onto the `/data` directory in the container.  Example runs:

```
docker run -it --rm --user=`id -u`:`id -g` --cpus="16" -v </your/current/directory>:/data tpesout/margin_polish:latest margin --help
docker run -it --rm --user=`id -u`:`id -g` --cpus="16" -v </your/current/directory>:/data tpesout/margin_polish:latest margin polish /data/input.bam /data/input.fasta /opt/margin/params/<model_name.json> -t 16 -o /data/output
docker run -it --rm --user=`id -u`:`id -g` --cpus="16" -v </your/current/directory>:/data tpesout/margin_polish:latest margin phase /data/input.bam /data/input.fasta /data/input.vcf /opt/margin/params/<model_name.json> -t 16 -o /data/output
```
