...
Poa *poa_realign(stList *bamChunkReads, stList *anchorAlignments, RleString *reference, PolishParams *polishParams) {
    ...
    for (int64_t i = 0; i < stList_length(bamChunkReads); i++) {
        ...
        if (anchorAlignments == NULL) {
            ...
            getAlignedPairsWithIndelsCroppingReference(reference, chunkRead->rleRead, chunkRead->forwardStrand,
                                                       stList_get(anchorAlignments, i),
...
void poa_printPhasedCSV(Poa *poa, FILE *fH,
    ...
    for (int64_t i = 0; i < stList_length(poa->nodes); i++) {
        ...
        double *baseWeightsHap1 = poaNode_getStrandSpecificBaseWeights(node, bamChunkReads,
                                                                       &totalWeightHap1, &totalPositiveWeightHap1,
                                                                       &totalNegativeWeightHap1, poa->alphabet,
                                                                       readsInHap1);
        ...
        double *baseWeightsHap2 = poaNode_getStrandSpecificBaseWeights(node, bamChunkReads,
                                                                       &totalWeightHap2, &totalPositiveWeightHap2,
                                                                       &totalNegativeWeightHap2, poa->alphabet,
                                                                       readsInHap2);
