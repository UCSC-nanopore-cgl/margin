...
static void msaView_printP2(MsaView *view, int64_t refStart, int64_t length,
    ...
    for (int64_t i = 0; i < length; i++) {
        indelLengthsToPrint[i] = msaView_getMaxPrecedingInsertLengthWithGivenCoverage(view, i + refStart,
                                                                                      minInsertCoverage);
    }
...
char seqCharBaseFn(int64_t seq, int64_t seqCoordinate, int64_t refCoordinate, void *extraArg) {
    ...
    return (refCoordinate >= 0 && view->refSeq[refCoordinate] == sequence[seqCoordinate]) ? '*'
                                                                                          : sequence[seqCoordinate];
}
...
void msaView_printRepeatCounts(MsaView *view, int64_t minInsertCoverage,
    ...
    msaView_printP(view, minInsertCoverage, refCharRepeatCountFn, seqCharRepeatCountFn,
                   (const void *[]) {refString, rleStrings}, fh);
}
