...
static char *polishParamsFile = "../params/polish/ont/r9.4/allParams.np.human.r94-g344.json";
...
void test_view(CuTest *testCase) {
    ...
    for (int64_t j = 0; j < seqNo; j++) {
        for (int64_t i = 0; i < refLength + 1; i++) {
            CuAssertIntEquals(testCase, precedingInsertLengthMatrix[(j * (refLength + 1)) + i],
                              msaView_getPrecedingInsertLength(view, i, j));
            CuAssertIntEquals(testCase, precedingInsertStartMatrix[(j * (refLength + 1)) + i],
                              msaView_getPrecedingInsertStart(view, i, j));
        }
    }
...
void test_viewExamples(CuTest *testCase) {
    ...
    for (int64_t example = 0; example < exampleNo; example++) {
        ...
        stList *refToTrueRefAlignment = getShiftedMEAAlignment(sX, sY, stList_construct(),
                                                               ...
                                                               params->polishParams->stateMachineForGenomeComparison, 0,
                                                               0, &alignmentScore);
